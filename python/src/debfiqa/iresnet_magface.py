'''
In order to ensure the consistency of model structure
the code of iresnet.py from magface original repo is used
'''
#from repo of magface: https://github.com/IrvingMeng/MagFace/blob/main/models/iresnet.py
import torch
...
class IResNet(nn.Module):
    ...
    def get_feature_sizes(self, x):
        '''
        Added by us
        For the one-time forward pass to get the feature sizes
        return: the feature sizes of multilayer features
        '''
        batch_size = x.size(0)
        x = self.conv1(x)
        x = self.bn1(x)
        ...
        l1_feat = self.layer1(x)
        l2_feat = self.layer2(l1_feat)
        l3_feat = self.layer3(l2_feat)
        ...
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        final_feat = self.features(x)

        return [l1_feat.view(batch_size, -1).size(1),
                l2_feat.view(batch_size, -1).size(1),
                l3_feat.view(batch_size, -1).size(1),
                l4_feat.view(batch_size, -1).size(1),
                final_feat.size(1)]
    ...
    def get_features(self, x):
        '''
        Added by us:
        get the multiple layer features for the demographic classifiers
        return: the multilayer features
        '''
        batch_size = x.size(0)
        x = self.conv1(x)
        x = self.bn1(x)
        ...
        return [l1_feat.view(batch_size, -1), 
                l2_feat.view(batch_size, -1), 
                l3_feat.view(batch_size, -1), 
                l4_feat.view(batch_size, -1), 
                final_feat]
